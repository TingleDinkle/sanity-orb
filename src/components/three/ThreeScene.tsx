...
const ThreeScene: React.FC<ThreeSceneProps> = ({ sanity, cameraAngles, collectiveData, collectiveAverage, onZoomIn, onZoomOut }) => {
  ...
  const orbitalStructuresRef = useRef<THREE.Group[]>([]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // MEGA DRONE SWARM - Large sophisticated drones orbiting Dyson sphere
      const orbitalStructures: THREE.Group[] = [];
      const orbitalCount = 16; // More drones for swarm effect
      const orbitalRadius = containmentRadius * 1.15; // Orbit just outside Dyson sphere
      
      // Create sophisticated mega drones
      for (let i = 0; i < orbitalCount; i++) {
        const droneGroup = new THREE.Group();
        
        // DRONE MAIN BODY - Sleek, aerodynamic design with texture
        const bodyLength = 1.0;
        const bodyWidth = 0.6;
        const bodyHeight = 0.4;
        
        // Main hull - elongated, streamlined with rounded edges
        const hullGeometry = new THREE.BoxGeometry(bodyLength, bodyHeight, bodyWidth, 8, 4, 6);
        const hullMaterial = new THREE.MeshStandardMaterial({
          color: metallicDark,
          metalness: 0.95,
          roughness: 0.12,
          emissive: metallicMedium,
        ...
        // Hull texture details - panel lines and seams
        const detailLineGeometry = new THREE.BoxGeometry(bodyLength * 1.1, 0.01, 0.01);
        const detailLineMaterial = new THREE.MeshStandardMaterial({
          color: metallicMedium,
          metalness: 0.9,
          roughness: 0.2
        });
        
        // Add detail lines along hull
        for (let line = 0; line < 3; line++) {
          const detailLine = new THREE.Mesh(detailLineGeometry, detailLineMaterial);
          detailLine.position.y = (line - 1) * 0.12;
          detailLine.position.z = bodyWidth / 2 + 0.01;
          droneGroup.add(detailLine);
        }
        
        // DRONE WINGS/STABILIZERS - Multiple wing structures with texture
        ...
        // Main wings (4 wings in X pattern) - larger and more detailed
        for (let w = 0; w < 4; w++) {
          const wingAngle = (w / 4) * Math.PI * 2;
          const wingGeometry = new THREE.BoxGeometry(0.5, 0.06, 0.2, 4, 1, 2);
          const wing = new THREE.Mesh(wingGeometry, wingMaterial);
          wing.position.x = Math.cos(wingAngle) * 0.35;
          wing.position.z = Math.sin(wingAngle) * 0.35;
          wing.rotation.y = wingAngle;
          wing.rotation.x = Math.PI / 5; // Angled wings
          droneGroup.add(wing);
          
          // Wing details - small panels
          const wingDetailGeometry = new THREE.BoxGeometry(0.15, 0.04, 0.12);
          const wingDetail = new THREE.Mesh(wingDetailGeometry, new THREE.MeshStandardMaterial({
            color: metallicLight,
            metalness: 0.85,
            roughness: 0.2
          }));
          wingDetail.position.set(0.2, 0, 0);
          wing.add(wingDetail);
        }
        
        // DRONE THRUSTERS - Engine pods with detailed texture
        const thrusterGeometry = new THREE.CylinderGeometry(0.1, 0.12, 0.25, 16);
        ...
        // Four thruster pods - larger and more detailed
        for (let t = 0; t < 4; t++) {
          const thrusterAngle = (t / 4) * Math.PI * 2 + Math.PI / 4;
          const thruster = new THREE.Mesh(thrusterGeometry, thrusterMaterial);
          thruster.position.x = Math.cos(thrusterAngle) * 0.4;
          thruster.position.z = Math.sin(thrusterAngle) * 0.4;
          thruster.position.y = -0.12;
          thruster.rotation.x = Math.PI / 2;
          droneGroup.add(thruster);
          
          // Thruster housing details
          const housingGeometry = new THREE.CylinderGeometry(0.12, 0.12, 0.08, 16);
          const housing = new THREE.Mesh(housingGeometry, new THREE.MeshStandardMaterial({
            color: metallicMedium,
            metalness: 0.9,
            roughness: 0.2
          }));
          housing.position.copy(thruster.position);
          housing.position.y += 0.1;
          housing.rotation.x = Math.PI / 2;
          droneGroup.add(housing);
          
          // Thruster glow - more intense
          const glowGeometry = new THREE.CylinderGeometry(0.07, 0.1, 0.2, 16);
        ...
        // DRONE SENSOR ARRAY - Front-facing sensors with detail
        const sensorGeometry = new THREE.BoxGeometry(0.18, 0.12, 0.25, 2, 2, 2);
        ...
        sensor.position.x = bodyLength / 2 + 0.12;
        droneGroup.add(sensor);
        
        // Sensor lens/details
        const lensGeometry = new THREE.SphereGeometry(0.06, 12, 12);
        ...
        for (let lens = 0; lens < 3; lens++) {
          ...
          lensMesh.position.x = bodyLength / 2 + 0.2;
          lensMesh.position.y = (lens - 1) * 0.08;
          droneGroup.add(lensMesh);
        }
        
        // DRONE DETAIL PANELS - Surface details for texture and realism
        ...
        // Add detail panels on hull - more organized
        ...
        for (let p = 0; p < panelPositions.length; p++) {
          const pos = panelPositions[p];
          const panelGeometry = new THREE.BoxGeometry(0.14, 0.1, 0.14, 2, 2, 2);
          const panel = new THREE.Mesh(panelGeometry, panelMaterial);
          panel.position.set(pos.x, pos.y, pos.z);
          droneGroup.add(panel);
          
          // Panel screws/details
          const screwGeometry = new THREE.CylinderGeometry(0.015, 0.015, 0.02, 8);
          ...
          for (let s = 0; s < 4; s++) {
            ...
            const screwX = (s % 2 === 0 ? -1 : 1) * 0.05;
            const screwZ = (s < 2 ? -1 : 1) * 0.05;
            screw.position.set(pos.x + screwX, pos.y, pos.z + screwZ);
            screw.rotation.x = Math.PI / 2;
            droneGroup.add(screw);
          }
        }
        
        // DRONE ANTENNA/COMMUNICATION ARRAY - More detailed
        const antennaGeometry = new THREE.CylinderGeometry(0.025, 0.025, 0.35, 10);
        ...
        for (let a = 0; a < 3; a++) {
          ...
          antenna.position.x = -bodyLength / 2 - 0.06;
          antenna.position.y = (a - 1) * 0.14;
          antenna.rotation.z = Math.PI / 10;
        ...
        // DRONE POWER CORE - Central energy source
        ...
        // DRONE ENERGY CONDUITS - Visible power lines
        const conduitGeometry = new THREE.CylinderGeometry(0.015, 0.015, 0.25, 8);
        ...
        for (let c = 0; c < 4; c++) {
          ...
          const conduitAngle = (c / 4) * Math.PI * 2;
          conduit.position.x = Math.cos(conduitAngle) * 0.2;
          conduit.position.z = Math.sin(conduitAngle) * 0.2;
          conduit.position.y = 0.05;
          conduit.rotation.z = Math.PI / 2;
          conduit.rotation.y = conduitAngle;
          droneGroup.add(conduit);
        }
        
        // Orbital parameters - different inclinations and phases
        const orbitInclination = (i / orbitalCount) * Math.PI * 0.6 - Math.PI * 0.3; // -30° to +30°
        const orbitPhase = (i / orbitalCount) * Math.PI * 2;
        const orbitSpeed = 0.05 + (i % 3) * 0.02; // Varying speeds
        ...
        droneGroup.userData = {
          ...
          orbitPhase,
          orbitSpeed,
          rotationSpeed: 0.1 + (i % 2) * 0.05,
          phase: Math.random() * Math.PI * 2,
          index: i
        };
        
        // Initial position
        droneGroup.position.x = orbitalRadius * Math.cos(orbitPhase) * Math.cos(orbitInclination);
        droneGroup.position.y = orbitalRadius * Math.sin(orbitInclination);
        droneGroup.position.z = orbitalRadius * Math.sin(orbitPhase) * Math.cos(orbitInclination);
        
        orbitalStructures.push(droneGroup);
        scene.add(droneGroup);
      }
      
      orbitalStructuresRef.current = orbitalStructures;
      ...
      const animate = (currentTime: number) => {
        ...
        // MEGA DRONE SWARM ANIMATION - Sophisticated drone behavior
        if (orbitalStructuresRef.current) {
          orbitalStructuresRef.current.forEach((drone) => {
            const data = drone.userData;
            
            // Orbital motion - complex 3D paths
            const orbitAngle = data.orbitPhase + t * data.orbitSpeed;
            
            // Calculate position on orbital plane
            const x = data.orbitRadius * Math.cos(orbitAngle) * Math.cos(data.orbitInclination);
            const y = data.orbitRadius * Math.sin(data.orbitInclination);
            const z = data.orbitRadius * Math.sin(orbitAngle) * Math.cos(data.orbitInclination);
            
            drone.position.set(x, y, z);
            
            // Drone orientation - faces direction of travel with slight banking
            // Look ahead in orbit direction
            const nextAngle = orbitAngle + deltaTime * data.orbitSpeed * 10;
            const nextX = data.orbitRadius * Math.cos(nextAngle) * Math.cos(data.orbitInclination);
            const nextY = data.orbitRadius * Math.sin(data.orbitInclination);
            const nextZ = data.orbitRadius * Math.sin(nextAngle) * Math.cos(data.orbitInclination);
            const lookTarget = new THREE.Vector3(nextX, nextY, nextZ);
            drone.lookAt(lookTarget);
            
            // Banking effect - tilt during turns
            const bankAngle = Math.sin(orbitAngle * 2) * 0.2;
            drone.rotation.z = bankAngle;
            
            // Rotate around own axis - drone rotation
            drone.rotation.y += deltaTime * data.rotationSpeed;
            
            // Thruster animation - pulsing glow
            const sanityState = sanityRef.current / 100.0;
            const thrusterPulse = Math.sin(t * (2.0 + sanityState * 1.0) + data.phase) * 0.3 + 0.7;
            
            // Update materials based on sanity state
            drone.traverse((child) => {
              if (child instanceof THREE.Mesh) {
                const material = child.material as THREE.MeshStandardMaterial | THREE.MeshBasicMaterial;
                
                if (material instanceof THREE.MeshStandardMaterial) {
                  // Emissive intensity varies with sanity
                  if (material.emissive) {
                    const emissiveIntensity = 0.1 + sanityState * 0.4;
                    material.emissive.copy(initialColor);
                    material.emissiveIntensity = emissiveIntensity;
                  }
                  
                  // Metalness and roughness respond to state
                  material.metalness = 0.85 + sanityState * 0.1;
                  material.roughness = 0.15 - sanityState * 0.05;
                } else if (material instanceof THREE.MeshBasicMaterial) {
                  // Thruster glows - intensity varies with sanity
                  const glowIntensity = 0.5 + sanityState * 0.5;
                  material.opacity = glowIntensity * thrusterPulse;
            ...
            // Chaos effects - drone instability
            if (chaos > 0.1) {
              const chaosOffset = Math.sin(t * 4.0 * chaos + data.index) * chaos * 0.15;
              drone.position.x += chaosOffset;
              drone.position.y += chaosOffset * 0.6;
              drone.position.z += chaosOffset;
              
              // Erratic movement
              const erratic = Math.sin(t * 6.0 * chaos + data.index) * chaos * 0.1;
              drone.rotation.x += erratic;
              drone.rotation.y += erratic * 0.5;
              
              // Flickering systems
              const flickerTime = t * 8.0 + data.index;
              const flicker = (Math.sin(flickerTime) > 0.3) ? 1.0 : 0.6;
              drone.children.forEach((child) => {
                if (child instanceof THREE.Mesh) {
                  const material = child.material as THREE.Material;
                  if (material instanceof THREE.MeshStandardMaterial || material instanceof THREE.MeshBasicMaterial) {
                    material.opacity = (material.opacity || 1.0) * flicker;
                  }
                }
              });
            }
